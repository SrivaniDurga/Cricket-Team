...
public class PlayerService implements PlayerRepository {
    ...
    public Player getPlayerById(int playerId){
        Player singlePlayer = team.get(playerId);
    ...
    @Override
    public Player addPlayer(Player singlePlayer){
        singlePlayer.setPlayerId(uniqueId);
        team.put(uniqueId , singlePlayer);
        uniqueId += 1;
        return singlePlayer;
    }
    @Override
    public Player updatePlayer(int playerId , Player singlePlayer){
        Player existingPlayer = team.get(playerId);
        if(existingPlayer == null){
            throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        }
        if(singlePlayer.getPlayerName() != null){
            existingPlayer.setPlayerName(singlePlayer.getPlayerName());
        }
        if(singlePlayer.getJerseyNumber() >0){
            existingPlayer.setJerseyNumber(singlePlayer.getJerseyNumber());
        }
        if(singlePlayer.getRole() != null){
            existingPlayer.setRole(singlePlayer.getRole());
        }
